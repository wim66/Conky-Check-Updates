...
local cache_file = "/tmp/checkupdates_cache.txt"
local aur_cache_file = "/tmp/aurupdates_cache.txt"
...
-- Cache vernieuwen voor reguliere updates
local function update_cache()
    ...
    if (os.time() - last_update) > cache_duration then
        os.execute("checkupdates > " .. cache_file .. " 2>/dev/null")
    end
end

-- Cache vernieuwen voor AUR-updates
local function update_aur_cache()
    local handle = io.popen("stat -c %Y " .. aur_cache_file .. " 2>/dev/null")
    local last_update = tonumber(handle:read("*a")) or 0
    handle:close()
    if (os.time() - last_update) > cache_duration then
        os.execute("yay -Qua > " .. aur_cache_file .. " 2>/dev/null")  -- Gebruik "paru -Qua" als je paru gebruikt
    end
end

-- Laden van pakketregels uit beide caches
local function load_package_lines()
    original_lines = {}

    -- Voeg reguliere updates toe
    ...
    -- Voeg AUR-updates toe
    local aur_file = io.open(aur_cache_file, "r")
    if aur_file then
        for line in aur_file:lines() do
            local package_name = line:match("^%S+")
            if package_name then
                if #package_name > max_line_length then
                    package_name = package_name:sub(1, max_line_length - 3) .. "..."
                end
                table.insert(original_lines, "[AUR] " .. package_name)
            end
        end
        aur_file:close()
    end
